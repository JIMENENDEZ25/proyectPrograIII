package com.umg.callcenter.system.atencion.mix.controller;
...
import com.umg.callcenter.system.atencion.mix.conexion.ConexionServidor;
import com.umg.callcenter.system.atencion.mix.modelo.Atencion;
import com.umg.callcenter.system.atencion.mix.modelo.Cliente;
...
//    public void setOnClienteFinalizado(Runnable callback) { this.onClienteFinalizado = callback; }
    ...
    public void cambiarModoAtencion(String nuevoModo) {
        if (!conexion.estaConectado()) {
            log("❌ No hay conexión para cambiar modo");
            return;
        }

        // ACTUALIZAR LA VARIABLE
        this.tipoAgente = nuevoModo;

        log("🔄 Cambiando modo a: " + nuevoModo.toUpperCase());

        JsonObject cambioModo = new JsonObject();
        cambioModo.addProperty("comando", "CAMBIAR_MODO");
        cambioModo.addProperty("modo", nuevoModo);
        cambioModo.addProperty("agente", nombreAgente);
        conexion.enviarComando(cambioModo);

        // Forzar actualización inmediata de la lista con el nuevo tipo
    ...
    public void actualizarListaClientes() {
        ...
        comando.addProperty("tipo", this.tipoAgente);  // Usar el tipo actual
        conexion.enviarComando(comando);
        System.out.println("[DEBUG] Solicitando lista para tipo: " + this.tipoAgente);
    }
    ...
    public void finalizarAtencion(String motivo, int duracion) {
        ...
        // NO limpiar clienteActual aquí - esperar confirmación
    }
